      ...
      <div ref={revealRef} className="relative z-10 w-full flex flex-col md:flex-row p-8 md:p-14 lg:p-20 min-h-screen items-center md:items-stretch gap-10">
        <div className="flex-1 min-w-0 flex flex-col justify-between pb-12 md:pb-8 w-full">
          <div className="flex items-center gap-3">
             ...
             <span className="font-mono text-[11px] font-bold text-white tracking-[0.2em] uppercase">SHINN.UK</span>
          </div>

          <div className="max-w-4xl lg:-translate-y-8 pr-12 text-left">
            <h1 className="text-[clamp(3.5rem,9.5vw,11.5rem)] font-black leading-[0.87] tracking-tighter text-white uppercase italic-none">
              SHINN <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-white/40 to-white/10" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.2)' }}>GEE</span>
            </h1>
            <p className="mt-8 font-mono text-[11px] text-white/40 uppercase tracking-[0.35em] max-w-sm leading-relaxed">
              Engineering immersive robotics and spatial logic through advanced code.
            </p>
          </div>
          ...
          <button ref={ctaRef} className="w-fit flex items-center gap-6 group lg:-translate-y-20">
             ...
             <span className="font-mono text-[11px] font-bold text-white uppercase tracking-[0.2em]">Explore Work</span>
          </button>
        </div>

        <div className="w-full md:w-80 lg:w-96 flex-shrink-0 flex flex-col gap-4 justify-center z-20">
          {[
            { id: "001", title: "AVAILABILITY", val: "SUMMER 2026", type: "progress" },
            { id: "002", title: "MEng ROBOTICS", val: "CLASS OF 2026", type: "data" },
            { id: "003", title: "EXPERTISE", val: "Creative Dev", type: "text" }
          ].map((item) => (
            <div key={item.id} className="command-cell backdrop-blur-md bg-white/5 border border-white/10 p-6 sm:p-7 block opacity-1 rounded-xl">
              <span className="font-mono text-[9px] text-white/25 uppercase tracking-widest block mb-3">{item.id} // {item.title}</span>
              {item.type === "progress" ? (
                <div className="flex flex-col gap-2 mt-2">
                  <h4 className="text-xl sm:text-2xl font-bold text-white tracking-tighter uppercase">{item.val}</h4>
                ...
                <div className="mt-4 flex flex-col gap-3 uppercase">
                  ...
                  <div className="flex justify-between text-[10px] font-mono text-white/50">
                    <span>Location</span>
                    <span className="text-white">UK / GLOBAL</span>
                  </div>
                </div>
              ) : (
                <h3 className="text-sm font-medium text-white/70 mt-3 leading-snug">
                  Transforming robotics into <span className="italic text-white">narrative apertures</span>.
                </h3>
              )}
            </div>
          ))}
